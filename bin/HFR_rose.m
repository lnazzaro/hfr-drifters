function varargout = HFR_rose(D,F,varargin)
...
%                an academic case.
%   06-08-2020 - added NSEW label options, removed percent labels (L
%                Nazzaro)
...
for i=1:length(vin)
  ...
  elseif isequal(vin{i},'incout')
    IncHiLow=vin{i+1};
  elseif isequal(vin{i},'nsewlabel')
    nsew=vin{i+1};
  end
end
...
elseif strcmp(nsew,'meteo')
    nsew={'0','180','90','-90'};
elseif strcmp(nsew,'math')
    nsew={'90','-90','0','180'};
else
    nsew={'NORTH','SOUTH','EAST','WEST'};
end
...
h(1)=text(-g-ri, 0,nsew{4}, 'VerticalAlignment','bottom',   'HorizontalAlignment','center', 'rotation',90, args{:});
h(2)=text( g+ri, 0,nsew{3}, 'VerticalAlignment','bottom',   'HorizontalAlignment','center','rotation',270,args{:});
h(3)=text( 0,-g-ri,nsew{2},'VerticalAlignment','top','HorizontalAlignment','center', args{:});
h(4)=text( 0, g+ri,nsew{1},'VerticalAlignment','bottom',   'HorizontalAlignment','center', args{:});
handles=[handles h];
